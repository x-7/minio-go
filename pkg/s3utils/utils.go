...
func IsVirtualHostSupported(endpointURL url.URL, bucketName string) bool {
	...
	return IsAmazonEndpoint(endpointURL) || IsGoogleEndpoint(endpointURL) || IsAliyunOSSEndpoint(endpointURL) || IsTecentCloudEndpoint(endpointURL)
}
...
// IsTecentCloudCOSEndpoint - Match if it is exactly TecentCloud OSS endpoint.
func IsTecentCloudEndpoint(endpointURL url.URL) bool {
